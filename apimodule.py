...
class host:
    ...
    def addhostgroup(usrdef_sship, hostname, groupname):
...
class network:
    ...
    def addnetwork(usrdef_sship, netname, netsub, netmask, netcolor):
    ...
    def addnetgroup(usrdef_sship, netname, groupname):
    ...
    def getallnetworks(usrdef_sship):
    ...
    def importaddnetwork(usrdef_sship, netname, netsub, netmask, netcolor, natset):
        natset = eval(natset)
    ...
    def importnetworks(usrdef_sship, filename):
        csvnets = open(filename, "r").read().split("\n")
        for line in csvnets:
            if not line:
                continue
            apiprep = line.split(';')
            network.importaddnetwork(usrdef_sship, apiprep[0], apiprep[1], apiprep[2], apiprep[3], apiprep[4])

    #Method to export host to csv file
    def exportnetworks(usrdef_sship):
...
class group:
    ...
    def addgroup(usrdef_sship, groupname):
    ...
    def addgroupgroup(usrdef_sship, addgroupname, groupname):
    ...
    def addgroupmembers(usrdef_sship, groupname, members):
        new_group_data = {'name':groupname, 'members':members}
        new_group_result = ac(usrdef_sship, 443,'add-group', new_group_data ,sid)

    #Method to import group from csv
    def importgroups(usrdef_sship, filename):
        ...
        for line in csvgroups:
            ...
            group.addgroupmembers(usrdef_sship, groupname[0], memberlist[0:-1])

    #Method to export host to csv file
    def exportgroups(usrdef_sship):
...
class policy:
    ...
    def importaddrules(usrdef_sship, num, name, src, dst, srv, act):
    ...
    def importrules(usrdef_sship, filename):
        ...
        for line in csvrules:
            ...
            try:
                ...
                srv = fullrule[4]
            act = fullrule[5]
            policy.importaddrules(usrdef_sship, num, name, src, dst, srv, act)

    #Method to get packages
    def getallpackages(usrdef_sship):
    ...
    def getalllayers(usrdef_sship, package):
    ...
    def exportrules(usrdef_sship, package, layer):
        #Retrieve Rulebase
...
class misc:
    ...
    def getalltargets(usrdef_sship):
        #Retrieve Targets
    ...
    def runscript(usrdef_sship, target, name, command):
    ...
    #Method to put file
    def putfile(usrdef_sship, target, path, name, contents):
